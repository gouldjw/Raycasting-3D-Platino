...
NSString * Raycasting3DTest$ModuleRequireFormat = @"(function(exports){"
...
@implementation Raycasting3DTestObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_raycasting3dtest);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Raycasting3DTest global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Raycasting3DTest" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_raycasting3dtest = [[Raycasting3DTestObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_raycasting3dtest];
	
	NSString *_raycasting3dtestNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _raycasting3dtestNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_raycasting3dtest valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Raycasting3DTest$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.raycasting3dtest.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.raycasting3dtest.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
