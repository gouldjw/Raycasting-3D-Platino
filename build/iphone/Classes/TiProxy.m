...
//Rare exceptions to indicate a bug in the _raycasting3dtest code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_raycasting3dtest = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_raycasting3dtest]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.raycasting3dtest.module"
